...
def train_calibrator(
    *,
    ...
    models = {}
    metrics = {}
    min_n = 5  # real-breath corpora are small but high-value
    for voc_id in sorted(voc_ids.unique()):
        ...
        if len(Xi) < min_n:
            continue
        if len(Xi) < 12:
            # too small for a holdout — fit all, report train MAE
            ...
            metrics[voc_id] = {
                "mae_log2fc": float(mean_absolute_error(yi, pred)),
                "r2": float(r2_score(yi, pred)) if len(np.unique(yi)) > 1 else None,
                "n_train": int(len(Xi)),
                "n_test": 0,
                "fit": "all_data_small_n",
            }
            models[voc_id] = model
            continue
    ...
    # Global fallback head for VOCs with too few disease labels
    if len(X) >= min_n:
        gbr = GradientBoostingRegressor(
            max_depth=3, learning_rate=0.08, n_estimators=150, random_state=random_state
        )
        gbr.fit(X, y)
        models["__global__"] = gbr
        metrics["__global__"] = {
            "mae_log2fc": float(mean_absolute_error(y, gbr.predict(X))),
            "n_train": int(len(X)),
            "n_test": 0,
            "fit": "global_fallback",
        }
    ...
    bundle = {
        ...
        "metrics": metrics,
        "version": "exhalepath-calibrator-real-1.0",

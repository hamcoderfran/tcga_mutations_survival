...
class AltBreathSources(DataSource):
    ...
    def harvest(self, *, offline: bool = False) -> dict[str, Path]:
        ...
        # Packaged peaks live under src/exhalepath/data/public_breath; repo also
        # may mirror under data/public_breath after harvest-public-breath.
        cbd_candidates = [
            self.root / "data" / "public_breath",
            self.root / "src" / "exhalepath" / "data" / "public_breath",
        ]
        try:
            from ..config import DATA_DIR

            cbd_candidates.insert(0, Path(DATA_DIR) / "public_breath")
        except Exception:  # noqa: BLE001
            pass
        cbd_dir = next(
            (p for p in cbd_candidates if (p / "Asthma_peaktable_ver3.csv").exists()),
            cbd_candidates[-1],
        )
